Function Get-AllAvailabilityGroupReplicas {
    [cmdletbinding()]
    Param(
...
"@
    ...
    try {
        $AGReplicas = Invoke-SqlCmd -TrustServerCertificate -ServerInstance $ServerInstance -query $query -Database master -QueryTimeout 180 -ErrorAction Stop
        return $AGReplicas
